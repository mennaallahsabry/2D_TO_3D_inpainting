...
def sparse_bilateral_filtering(
    ...
    for i in range(num_iter):
        ...
        # Create logical masks based on boolean conditions
        u_mask = u_over > 0
        b_mask = b_over > 0
        l_mask = l_over > 0
        r_mask = r_over > 0

        # Use masks to set values to zero in vis_image
        vis_image[u_mask] = np.array([0, 0, 0])
        vis_image[b_mask] = np.array([0, 0, 0])
        vis_image[l_mask] = np.array([0, 0, 0])
        vis_image[r_mask] = np.array([0, 0, 0])
